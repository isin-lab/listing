...
function RenderLevel (props) {
  const { item } = props;
  if (item?.quantity > 20) {
    return (<p className="item-quantity level-high">{item.quantity}</p>)
  } else if (item?.quantity <= 20) {
    return (<p className="item-quantity level-medium">{item.quantity}</p>)
  } else if (item?.quantity <= 10) {
    return (<p className="item-quantity level-low">{item.quantity}</p>)
  } 
}
